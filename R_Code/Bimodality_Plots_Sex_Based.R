...
for (i in seq (2, 2924, 9)){
  ...
  if (j < 2924) {
    ...
    Olink_prot_1 <- df_Olink_wellness_merged[,i:2924]
    Olink_prot <- as.data.frame (cbind (Type = df_Olink_wellness_merged$Type, Olink_prot_1), na.rm = TRUE)
  }
  
  my_plots <- lapply(names(Olink_prot [,-1]), function(var_x){
    p1 <- 
      ggplot(subset (Olink_prot, Olink_prot$Type == "UKBB-Olink-Male")) +  aes_string(x = var_x) +
    ...
    p2 <- 
      ggplot(subset (Olink_prot, Olink_prot$Type == "Wellness-Olink-Male")) +  aes_string(x = var_x) +
      geom_histogram(aes(y = after_stat(density)), bins = 30, fill = "gray", color = "white") +
    ...
    p3 <- 
      ggplot(subset (Olink_prot, Olink_prot$Type == "UKBB-Olink-Female")) +  aes_string(x = var_x) +
      geom_histogram(aes(y = after_stat(density)), bins = 30, fill = "gray", color = "white") +
      geom_density(color = "green", linewidth = 1) +
      theme_minimal() +
      labs(title = "UKBB-Female", y = "Density")
    p3 <- p3 + theme(plot.title = element_text(family = "serif", size=8, face = "bold"),
                     axis.title.x = element_text(family = "serif", size=8),
                     axis.title.y = element_text(family = "serif", size=8),
                     axis.text.x = element_text(family = "serif", size=5),
                     axis.text.y = element_text(family = "serif", size=5),
    ...
    p4 <- 
      ggplot(subset (Olink_prot, Olink_prot$Type == "Wellness-Olink-Female")) +  aes_string(x = var_x) +
      geom_histogram(aes(y = after_stat(density)), bins = 30, fill = "gray", color = "white") +
      geom_density(color = "purple", linewidth = 1) +
      theme_minimal() +
      labs(title = "ISB-Female", y = "Density")
    p4 <- p4 + theme(plot.title = element_text(family = "serif", size=8, face = "bold"),
                     axis.title.x = element_text(family = "serif", size=8),
                     axis.title.y = element_text(family = "serif", size=8),
                     axis.text.x = element_text(family = "serif", size=5),
                     axis.text.y = element_text(family = "serif", size=5),
