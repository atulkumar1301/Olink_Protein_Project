...
plot_dir <- "~/Library/CloudStorage/OneDrive-UniversityofEasternFinland/Projects/Merja_Sui_Olink_Project/Modality_Plot_Sex_Based/"

# List all individual PDF files generated by the loop
# We use mix of regex anchors to ensure we only catch the generated files
pdf_files <- list.files(path = plot_dir, pattern = "^[0-9]+_Modality_Plot\\.pdf$", full.names = TRUE)

# Sort the files numerically so Page 2 comes before Page 10
# Extract numbers from filenames, convert to numeric, and sort them properly
file_numbers <- as.numeric(gsub(".*?/([0-9]+)_Modality_Plot\\.pdf", "\\1", pdf_files))
pdf_files <- pdf_files[order(file_numbers)]

# Define the path for the final merged multi-page PDF
combined_output <- file.path("~/Library/CloudStorage/OneDrive-UniversityofEasternFinland/Projects/Merja_Sui_Olink_Project/Modality_Plot_All_Sex_Based.pdf")

# Merge all files into one master document
qpdf::pdf_combine(input = pdf_files, output = combined_output)
